...
func (zones *zones) UnmarshalJSON(data []byte) error {
	zones.Zones = []*Zone{}
	err := json.Unmarshal(data, &zones.Zones)
	if err != nil {
		return err
	}
	zones.sort()
...
func (zones zones) MarshalJSON() ([]byte, error) {
	return json.Marshal(zones.Zones)
}

// Sorts zones using DNS order.
func (zones *zones) sort() {
	slices.SortFunc(zones.Zones, func(zone1, zone2 *Zone) int {
...
func (zones *zones) binarySearch(name string) *Zone {
	if index, ok := slices.BinarySearchFunc(zones.Zones, name, func(zone1 *Zone, name string) int {
		return storkutil.CompareNames(zone1.Name, name)
	}); ok {
		return zones.Zones[index]
	}
...
func (zones *zones) GetZone(name string) *Zone {
	if len(zones.Zones) < binaryZoneSearchThreshold {
