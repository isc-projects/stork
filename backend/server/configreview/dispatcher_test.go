...
// Test that the dispatcher doesn't fetch the subject daemon from the database
// if all references are available.
func TestEnsureSubjectDaemonHasReferencedEntriesAllOK(t *testing.T) {
	// Arrange
	daemon := &dbmodel.Daemon{
		ID:        42,
	...
	ctx := newReviewContext(nil, daemon, Triggers{ConfigModified}, nil)
...
// Test that the dispatcher fetches the subject daemon from the database
// if any references is unavailable.
func TestEnsureSubjectDaemonHasReferencedEntries(t *testing.T) {
	// Arrange
	db, _, teardown := dbtest.SetupDatabaseTestCase(t)
	defer teardown()
	...
	daemons, _ := dbmodel.AddApp(db, app)
	daemon := daemons[0]
	daemon, _ = dbmodel.GetDaemonByID(db, daemon.ID)

	ctx := newReviewContext(db, daemon, Triggers{ConfigModified}, nil)

	verifySubjectDaemon := func(t *testing.T) {
		require.NotNil(t, ctx.subjectDaemon.KeaDaemon)
		require.NotNil(t, ctx.subjectDaemon.App)
		require.NotNil(t, ctx.subjectDaemon.App.Machine)
	}
	...
	t.Run("all OK", func(t *testing.T) {
		...
		// Assert
		require.NoError(t, err)
		verifySubjectDaemon(t)
	})

	t.Run("missing KeaDaemon", func(t *testing.T) {
		ctx.subjectDaemon.KeaDaemon = nil
	...
	t.Run("missing machine", func(t *testing.T) {
		ctx.subjectDaemon.App.Machine = nil
