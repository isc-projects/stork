...
// Test parsing the incoming transfer status message indicating a failed connection.
func TestXfrTrackerParseIncomingTransferStatusFailed(t *testing.T) {
	xfrState := parseTransferLogLine("23-Feb-2026 10:41:27.147 0x7ffffb63b000: transfer of 'bind9.example.org/IN' from 172.24.0.53#53: Transfer status: connection refused")
	require.NotNil(t, xfrState)
	...
	require.Equal(t, bind9xfr.TimeFormatBind9, xfrState.TimeFormat)
	require.Equal(t, "Transfer status: connection refused", xfrState.Message)
}

// Test parsing the incoming transfer status message indicating that the zone is up to date.
func TestXfrTrackerParseIncomingTransferStatusUpToDate(t *testing.T) {
	xfrState := parseTransferLogLine("23-Feb-2026 10:41:27.147 0x7ffffb63b000: transfer of 'bind9.example.org/IN' from 172.24.0.53#53: Transfer status: up to date")
	require.NotNil(t, xfrState)
	require.Equal(t, bind9xfr.StatusUpToDate, xfrState.Status)
	require.Equal(t, "_default", xfrState.ViewName)
	require.Equal(t, "bind9.example.org", xfrState.ZoneName)
	require.Zero(t, xfrState.Serial)
	require.Empty(t, xfrState.Client)
	...
	require.NotZero(t, xfrState.CompletionTime)
	require.Equal(t, bind9xfr.TimeFormatBind9, xfrState.TimeFormat)
	require.Equal(t, "Transfer status: up to date", xfrState.Message)
}
...
func TestXfrTrackerParseIncomingTransferCompleted(t *testing.T) {
	...
	require.Equal(t, "_default", xfrState.ViewName)
	require.Equal(t, "bind9.example.org", xfrState.ZoneName)
	require.NotNil(t, xfrState.Serial)
...
func TestXfrTrackerFeed(t *testing.T) {
	...
	open := xfrTracker.getOpenZoneTransfers()
	require.Len(t, open, 1)
	require.Equal(t, "bind.example.org", open[0].ZoneName)
	require.Equal(t, bind9xfr.StatusMessage, open[0].Status)
	require.Equal(t, "127.0.0.1", open[0].Client)
	require.Equal(t, "", open[0].Server)
	require.NotZero(t, open[0].StartTime)
	require.Zero(t, open[0].CompletionTime)
	require.Equal(t, "setting up zone transfer: failed", open[0].Message)

	// There are four closed zone transfers.
	closed := xfrTracker.getClosedZoneTransfers()
	require.Len(t, closed, 4)

	require.Equal(t, "drop.rpz.example.com", closed[0].ZoneName)
	require.Equal(t, bind9xfr.StatusCompleted, closed[0].Status)
	require.NotZero(t, closed[0].StartTime)
	require.NotZero(t, closed[0].CompletionTime)
	require.Equal(t, "172.24.0.53", closed[0].Server)
	require.EqualValues(t, closed[0].MessagesCount, 1)
	require.EqualValues(t, closed[0].RecordsCount, 5)
	require.EqualValues(t, closed[0].BytesCount, 294)
	require.Equal(t, 14*time.Millisecond, closed[0].Duration)
	require.NotNil(t, closed[0].Serial)
	require.EqualValues(t, 201702121, *closed[0].Serial)
	require.Equal(t, "Transfer completed: 1 messages, 5 records, 294 bytes, 0.014 secs (21000 bytes/sec) (serial 201702121)", closed[0].Message)

	require.Equal(t, ".", closed[1].ZoneName)
	require.Equal(t, bind9xfr.StatusCompleted, closed[1].Status)
	require.NotZero(t, closed[1].StartTime)
	require.NotZero(t, closed[1].CompletionTime)
	require.Equal(t, "192.5.5.241", closed[1].Server)
	require.EqualValues(t, closed[1].MessagesCount, 123)
	require.EqualValues(t, closed[1].RecordsCount, 24872)
	require.EqualValues(t, closed[1].BytesCount, 1321903)
	require.Equal(t, 335*time.Millisecond, closed[1].Duration)
	require.NotNil(t, closed[1].Serial)
	require.EqualValues(t, 2026041600, *closed[1].Serial)
	require.Equal(t, "Transfer completed: 123 messages, 24872 records, 1321903 bytes, 0.335 secs (3945979 bytes/sec) (serial 2026041600)", closed[1].Message)

	require.Equal(t, "drop.rpz.example.com", closed[2].ZoneName)
	require.Equal(t, bind9xfr.StatusCompleted, closed[2].Status)
	require.NotZero(t, closed[2].StartTime)
	require.NotZero(t, closed[2].CompletionTime)
	require.EqualValues(t, closed[2].MessagesCount, 1)
	require.EqualValues(t, closed[2].RecordsCount, 5)
	require.EqualValues(t, closed[2].BytesCount, 199)
	require.Equal(t, 3*time.Millisecond, closed[2].Duration)
	require.NotNil(t, closed[2].Serial)
	require.EqualValues(t, 201702121, *closed[2].Serial)
	require.Equal(t, "AXFR ended: 1 messages, 5 records, 199 bytes, 0.003 secs (66333 bytes/sec) (serial 201702121)", closed[2].Message)

	require.Equal(t, ".", closed[3].ZoneName)
	require.Equal(t, bind9xfr.StatusCompleted, closed[3].Status)
	require.NotZero(t, closed[3].StartTime)
	require.NotZero(t, closed[3].CompletionTime)
	require.EqualValues(t, closed[3].MessagesCount, 79)
	require.EqualValues(t, closed[3].RecordsCount, 24872)
	require.EqualValues(t, closed[3].BytesCount, 1320233)
	require.Equal(t, 52*time.Millisecond, closed[3].Duration)
	require.NotNil(t, closed[3].Serial)
	require.EqualValues(t, 2026041600, *closed[3].Serial)
	require.Equal(t, "AXFR ended: 79 messages, 24872 records, 1320233 bytes, 0.052 secs (25389096 bytes/sec) (serial 2026041600)", closed[3].Message)
}

// Test that existing closed and open zone transfers can be returned, and the
...
func TestXfrTrackerFollow(t *testing.T) {
	...
	closed, open, followChan := xfrTracker.follow(ctx)
	require.Len(t, slices.Collect(closed), 4)
	require.Len(t, slices.Collect(open), 2)
...
func TestXfrTrackerFollowReconnect(t *testing.T) {
	...
	// new values.
	closed, open, followChan2 := xfrTracker.follow(ctx)
	require.Len(t, slices.Collect(closed), 7)
	require.Len(t, slices.Collect(open), 1)
...
func TestXfrTrackerFeedStartedConnected(t *testing.T) {
	...
	transfers := xfrTracker.getOpenZoneTransfers()
	require.Len(t, transfers, 1)
	require.Equal(t, ".", transfers[0].ZoneName)
	require.Equal(t, bind9xfr.StatusStarted, transfers[0].Status)
...
func TestXfrTrackerFeedStartedCompleted(t *testing.T) {
	...
	require.Equal(t, ".", closed[0].ZoneName)
	require.Equal(t, bind9xfr.StatusCompleted, closed[0].Status)
	require.NotZero(t, closed[0].StartTime)
	require.NotZero(t, closed[0].CompletionTime)
}
...
func TestXfrTrackerFeedCompletedBeforeStarted(t *testing.T) {
	...
	closed := xfrTracker.getClosedZoneTransfers()
	require.Len(t, closed, 1)
	require.Equal(t, "example.com", closed[0].ZoneName)
	require.Equal(t, bind9xfr.StatusCompleted, closed[0].Status)
	require.Zero(t, closed[0].StartTime, time.Time{})
	require.NotZero(t, closed[0].CompletionTime)
}

// Test that up to date transfer status is updated with the statistics
// received after the transfer status message but the status is not
// changed when the statistics are received.
func TestXfrTrackerFeedStartedUpToDateStatistics(t *testing.T) {
	xfrTracker := newXfrTracker(nil)
	xfrTracker.feed("23-Feb-2026 10:41:27.071 zone ./IN: Transfer started.")
	xfrTracker.feed("23-Feb-2026 10:41:27.141 @0x7ffffaa28c00 172.24.0.54#34961: transfer of './IN': Transfer status: up to date")
	transfers := xfrTracker.getOpenZoneTransfers()
	require.Len(t, transfers, 1)
	require.Equal(t, ".", transfers[0].ZoneName)
	require.Equal(t, bind9xfr.StatusUpToDate, transfers[0].Status)
	xfrTracker.feed("23-Feb-2026 10:41:27.141 @0x7ffffaa28c00 172.24.0.54#34961: transfer of './IN': Transfer completed 1 messages, 5 records, 294 bytes, 0.004 secs (73500 bytes/sec) (serial 201702121)")
	closed := xfrTracker.getClosedZoneTransfers()
	require.Len(t, closed, 1)
	require.Equal(t, ".", closed[0].ZoneName)
	require.Equal(t, bind9xfr.StatusUpToDate, closed[0].Status)
	require.NotZero(t, closed[0].StartTime)
	require.NotZero(t, closed[0].CompletionTime)
	require.EqualValues(t, 1, closed[0].MessagesCount)
	require.EqualValues(t, 5, closed[0].RecordsCount)
	require.EqualValues(t, 294, closed[0].BytesCount)
	require.Equal(t, 4*time.Millisecond, closed[0].Duration)
	require.NotNil(t, closed[0].Serial)
	require.EqualValues(t, 201702121, *closed[0].Serial)
	require.Equal(t, "Transfer status: up to date", closed[0].Message)
}

// Test that failed transfer status is updated with the statistics
// received after the transfer status message but the status is not
// changed when the statistics are received.
func TestXfrTrackerFeedStartedFailedStatistics(t *testing.T) {
	xfrTracker := newXfrTracker(nil)
	xfrTracker.feed("23-Feb-2026 10:41:27.071 zone ./IN: Transfer started.")
	xfrTracker.feed("23-Feb-2026 10:41:27.141 @0x7ffffaa28c00 172.24.0.54#34961: transfer of './IN': Transfer status: connection refused")
	transfers := xfrTracker.getOpenZoneTransfers()
	require.Len(t, transfers, 1)
	require.Equal(t, ".", transfers[0].ZoneName)
	require.Equal(t, bind9xfr.StatusFailed, transfers[0].Status)
	xfrTracker.feed("23-Feb-2026 10:41:27.141 @0x7ffffaa28c00 172.24.0.54#34961: transfer of './IN': Transfer completed 1 messages, 5 records, 294 bytes, 0.004 secs (73500 bytes/sec) (serial 201702121)")
	closed := xfrTracker.getClosedZoneTransfers()
	require.Len(t, closed, 1)
	require.Equal(t, ".", closed[0].ZoneName)
	require.Equal(t, bind9xfr.StatusFailed, closed[0].Status)
	require.NotZero(t, closed[0].StartTime)
	require.NotZero(t, closed[0].CompletionTime)
	require.EqualValues(t, 1, closed[0].MessagesCount)
	require.EqualValues(t, 5, closed[0].RecordsCount)
	...
	require.NotNil(t, closed[0].Serial)
	require.EqualValues(t, 201702121, *closed[0].Serial)
	require.Equal(t, "Transfer status: connection refused", closed[0].Message)
}

// Test that for an outgoing transfer, the failed status is recorded in the
// closed transfers list, so that it wouldn't be updated by any subsequent
// messages.
func TestXfrTrackerFeedOutgoingStartedFailed(t *testing.T) {
	xfrTracker := newXfrTracker(nil)
	xfrTracker.feed("16-Apr-2026 12:08:59.967 client @0x7ffff983b000 127.0.0.1#56830 (drop.rpz.example.com): transfer of 'drop.rpz.example.com/IN': AXFR started (serial 201702121)")
	xfrTracker.feed("16-Apr-2026 12:08:59.969 client @0x7ffff983b000 127.0.0.1#56830 (drop.rpz.example.com): transfer of 'drop.rpz.example.com/IN': AXFR aborted")
	closed := xfrTracker.getClosedZoneTransfers()
	require.Len(t, closed, 1)
	require.Equal(t, "drop.rpz.example.com", closed[0].ZoneName)
	require.Equal(t, bind9xfr.StatusFailed, closed[0].Status)
	require.NotZero(t, closed[0].StartTime)
	require.NotZero(t, closed[0].CompletionTime)
	require.Equal(t, "AXFR aborted", closed[0].Message)
}
...
func TestXfrTrackerLimits(t *testing.T) {
	...
	require.Equal(t, "1.example.com", open[0].ZoneName)
	require.Equal(t, "2.example.com", open[1].ZoneName)
	require.Equal(t, "3.example.com", open[2].ZoneName)
	require.Empty(t, xfrTracker.getClosedZoneTransfers())

	xfrTracker.feed(fmt.Sprintf(startedTemplate, 4))
	open = xfrTracker.getOpenZoneTransfers()
	require.Len(t, open, 3)
	require.Equal(t, "2.example.com", open[0].ZoneName)
	require.Equal(t, "3.example.com", open[1].ZoneName)
	require.Equal(t, "4.example.com", open[2].ZoneName)
	require.Empty(t, xfrTracker.getClosedZoneTransfers())
	...
	closed := xfrTracker.getClosedZoneTransfers()
	require.Len(t, closed, 1)
	require.Equal(t, "3.example.com", closed[0].ZoneName)
	open = xfrTracker.getOpenZoneTransfers()
	require.Len(t, open, 2)
	require.Equal(t, "2.example.com", open[0].ZoneName)
	require.Equal(t, "4.example.com", open[1].ZoneName)

	xfrTracker.feed(fmt.Sprintf(endedTemplate, 2))
	closed = xfrTracker.getClosedZoneTransfers()
	require.Len(t, closed, 2)
	require.Equal(t, "3.example.com", closed[0].ZoneName)
	require.Equal(t, "2.example.com", closed[1].ZoneName)
	open = xfrTracker.getOpenZoneTransfers()
	require.Len(t, open, 1)
	require.Equal(t, "4.example.com", open[0].ZoneName)

	xfrTracker.feed(fmt.Sprintf(startedTemplate, 5))
	open = xfrTracker.getOpenZoneTransfers()
	require.Len(t, open, 2)
	require.Equal(t, "4.example.com", open[0].ZoneName)
	require.Equal(t, "5.example.com", open[1].ZoneName)
	closed = xfrTracker.getClosedZoneTransfers()
	require.Len(t, closed, 2)
	...
	require.Equal(t, "3.example.com", closed[0].ZoneName)
	require.Equal(t, "2.example.com", closed[1].ZoneName)
	require.Equal(t, "5.example.com", closed[2].ZoneName)
	open = xfrTracker.getOpenZoneTransfers()
	require.Len(t, open, 1)
	require.Equal(t, "4.example.com", open[0].ZoneName)
	...
	closed = xfrTracker.getClosedZoneTransfers()
	require.Len(t, closed, 3)
	require.Equal(t, "2.example.com", closed[0].ZoneName)
	require.Equal(t, "5.example.com", closed[1].ZoneName)
	require.Equal(t, "4.example.com", closed[2].ZoneName)
	open = xfrTracker.getOpenZoneTransfers()
