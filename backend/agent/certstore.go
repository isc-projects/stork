...
// Handy interface to pass all certificate paths to the CertStore constructor.
// It is used in tests to pass the paths of the temporary, self-signed
// certificates.
...
// Creates a new certPaths structure with paths pointed to the given directory.
func newCertPaths(dir string) certPaths {
	return certPaths{
		...
		fingerprintPath: filepath.Join(dir, "server-cert.sha256"),
	}
}
...
func NewCertStoreDefault() *CertStore {
	return newCertStore(newCertPaths("/var/lib/stork-agent"))
}

// Constructs a new cert store instance with paths pointed to the given
// directory.
func newCertStore(paths certPaths) *CertStore {
	return &CertStore{
		keyPEMPath:                paths.keyPath,
		certPEMPath:               paths.certPath,
		rootCAPEMPath:             paths.caPath,
		agentTokenPath:            paths.tokenPath,
		serverCertFingerprintPath: paths.fingerprintPath,
	}
}
