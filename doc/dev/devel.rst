...
Stork system tests have been moved to a separate repository.
Please refer to `stork-tests <https://gitlab.isc.org/isc-projects/stork-tests>`_ project documentation
for instructions on running the system tests.
...
backend and one for the frontend. The first set is defined in the ``backend/daemoncfg/stdoptiondef4.go``
and ``backend/daemoncfg/stdoptiondef6.go`` files using the Golang syntax. The second set is
