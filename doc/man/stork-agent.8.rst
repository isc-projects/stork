...
``--env-file``
   Read the environment variables from the environment file; applicable only if the flag is set; accepts an optional path to the env file. The default is ``/etc/stork/agent.env``.
