...
``--puller-interval``
   Default interval used by pullers fetching data from Kea. If not provided the recommended values for each puller are used. ``[$STORK_SERVER_DEFAULT_PULLER_INTERVAL]``
