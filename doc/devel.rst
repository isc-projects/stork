...
backend and one for the frontend. First is defined in the ``backend/appcfg/stdoptiondef6.go```
file using the Golang syntax. Second is defined in the ``webui/src/app/std-option-defs6.ts``
file using the Typescript syntax. These files should not be modified directly. They are
generated by the ``stork-code-gen`` tool provided with the Stork source code.

To add or modify option definitions, edit the ``codegen/std_dhcpv6_option_def.json`` file.
It includes the definitions of all standard DHCP options in the portable JSON format.
This is the same format in which the definitions are specified in Kea. Once you
update the definitions, build and run the code-generating tool:
