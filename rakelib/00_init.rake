...
def find_dependency_tasks(file, extra_prerequisites=[])
    ...
    prerequisites_tasks.each do |t|
        ...
        # directory or GOBIN. They are autogenerated files, not external
        # dependencies. GOBIN is where `go install`-based tools land; it's
        # normally outside the project directory (e.g. ~/go/bin), but CI
        # (and some local setups) may point it inside the project, so it
        # needs the same allowance as tools_dir.
        t_path = t.name
        if t.name.include?("/")
            ...
            if t_path_abs.start_with?(project_dir_abs) && !t_path_abs.start_with?(tools_dir_abs) && !t_path_abs.start_with?(GOBIN_ABS)
                # In-project file task that isn't in the tools directory or GOBIN.
                next
            end
        end
...
GOBIN_ABS = File.expand_path(gobin)
