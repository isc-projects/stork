...
# recognized by the existing :@manual_install variable. Tasks explicitly
# marked with mark_generated are never considered a dependency, regardless
# of their class.
def is_dependency_task(t)
    return false if t.instance_variable_get(:@generated)
    t.class == Rake::FileTask || !t.instance_variable_get(:@manual_install).nil?
end

# Marks a file task as generated by the build rather than an external
# dependency.
def mark_generated(task_name)
    Rake::Task[task_name].instance_variable_set(:@generated, true)
    task_name
end
