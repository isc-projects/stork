...
# The workaround is to strip the read-only properties from the file provided to
# a generator of the GoSwagger server code.
swagger_server_file = 'api/swagger-server.yaml'

SWAGGER_FILE = 'api/swagger.yaml'
swagger_api_files = FileList['api/*.yaml'].exclude(SWAGGER_FILE).exclude(swagger_server_file)
...
file swagger_server_dir => [GOSWAGGER, swagger_server_file] do
    swagger_abs = File.expand_path(swagger_server_file)
