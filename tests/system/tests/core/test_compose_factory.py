...
from tests.core.commons import subprocess_result_mock, fake_compose_detector


def test_create_compose():
    compose = create_docker_compose(compose_detector=fake_compose_detector)
...
def test_create_compose_uses_environment_variables(patch):
    ...
    compose = create_docker_compose(env_vars=source_env_vars, compose_detector=fake_compose_detector)
    compose.up()
